...
def get_iterator2(
    ...
    src_dataset = src_dataset.map(lambda filename, duration, label: tuple(tf.py_func(
        fbank_from_file, [filename, duration, label], [
            tf.float32, tf.string, tf.float32, tf.string]
    )), num_parallel_calls=num_parallel_calls).prefetch(output_buffer_size)

    src_dataset = src_dataset.map(
        lambda src, filename, duration, label: (
            src, tf.size(src), filename, duration, label)
    ...
    def batching_func(x):
        return x.padded_batch(
            ...
            padded_shapes=(
                tf.TensorShape([None, None, None]),  # src
    ...
    def key_func(unused_1, src_len, filename, duration, label):
        ...
        # sentence.
        bucket_id = tf.maximum(src_len // 3 // 41 // bucket_width, 1)
...
#     with open("./aishell_train_abs.json", 'r') as f:
#         for i, line in enumerate(f.readlines()):
#             d = json.loads(line)
#             files.append(d.get("key"))
#             durations.append(d.get("duration"))
#             labels.append(d.get("text"))
